...
```mermaid
graph TD
    A[사용자 입력: 소프라노 멜로디 시퀀스] --> B{Neural Engine 활성화 여부}
    B -- CUDA GPU 지원 및 가중치 존재 시 --> C[NeuralBachEngine 추론 가동]
    B -- CPU 전용 또는 가중치 부재 시 --> D[FugueEngine 폴백 가동]
    
    C --> E[Gibbs Sampling Iterative Refinement 루프 진입]
    E --> F[로마자 화성 기호에 기반한 화성적 개연성 정교화]
    F --> G[소프라노/알토/테너/베이스 SATB 생성 및 정밀 보정]
    
    D --> H[기존 규칙 기반 카운터포인트 대위법 화성 합성]
...
### 3. Rule-based Counterpoint Engine: FugueEngine

딥러닝 모델의 가중치를 불러올 수 없거나 연산 자원이 부족한 경량 환경을 지원하기 위해, 수학적 대위법 이론에 입각한 규칙 기반 생성 엔진이 유기적으로 가동됩니다.

- 메커니즘:
  - 입력 선율 분석: 입력된 소프라노 선율의 최고점/최저점 및 현재 조성(Key) 정보를 추출합니다.
  - 모방 대위법 (Imitative Counterpoint): 5도 위 또는 4도 아래 등의 음정 간격으로 소프라노 주제를 모방하여 부성부(알토, 테너)를 계산합니다.
  - 종지부(Cadence) 장치: 곡의 마무리를 알리는 자연스러운 도미넌트-토닉 화성 종지를 시간 및 조성 분석 결과에 맞춰 수학적으로 강제 배치합니다.
